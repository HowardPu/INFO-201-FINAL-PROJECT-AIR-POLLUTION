...
### What is _AQI_
<center>
![AQI](https://www.airnow.gov/images/aqilogo_sm.gif){width=20%}
</center>
...
### Why we use AQI
> Local,regional,and national governments decide how to disseminate monitor measurements to the public and the preferred way to communicate is via a color-coded Air Quality Index (AQI) that is easy for the public to understand

***

## How to inteperate our four data visualization?

### Pie Plot
> Our pie plot is constructed using data of each pollutant (NO2, CO, SO2, O3) from each state. The graph shows the percentage of each pollutant's AQI value among the AQI value of all the pollutants in that state among the years from 2000 to 2016. Users can select the state they want to look at through the _select box_ on the left side of the page. The plot will enable the users to find the most threatening air pollutant in each state.

### Map Plot
> Our map plot is constructed using data of each state in each year. The graph shows the severely the air pollution is in each state in each year by using the AQI value of that state in each year. Users can select the year they want to look at through the _slider_ on the left side of the page. The plot will enable the users to compare the condition of each state's air quality more directly. 

### Scatter Plot
> Our scatter plot is constructed using data of each air pollutant among the years from 2001 - 2016. The graph shows the relationship of the two air pollutant of interests in the selected year. Users can select two air pollutants they are interested in and the year they want to investigate on through the _select box_. The plot will enable the users to get a understanding of the relationship of two air pollutant in each year. 

### Bar Plot
> Our bar plot is constructed using data of each air pollutant among the years from 2000 to 2016. The graph shows the total AQI of air pollution in each year and the contrast between each air pollutant. Users can select the air pollutants they want to look at through the _check box_ and year they want to look at through the _slide bar_ on the left side of the page. The plot will enable the users to get a general understanding of how severely the air pollution is in each year in each month and what is the component of the air pollution is.
